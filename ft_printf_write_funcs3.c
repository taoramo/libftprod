...
void	printf_write_int_us(t_printf *tab)
{
	...
	intlength = int_str_size_printf_us((unsigned long)tab -> arg, tab);
	pad = 32;
	...
	if (!(tab -> dot == 1 && tab -> precision == 0
			&& (unsigned long)tab -> arg == 0))
		ft_putnbr_fd_us((unsigned long)tab -> arg, 1);
...
{
	if (tab -> alt && (unsigned long)tab -> arg != 0 && tab -> type == 'x')
